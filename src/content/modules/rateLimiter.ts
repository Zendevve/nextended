...
/**
 * ADR-0003 — Safe Floor on Bulk Run pacing. Free-account inter-download
 * pacing can never be configured below this floor; the 200-downloads-per-
 * 5-minutes cooldown stays always-on regardless. Kept as data (not branches)
 * so future premium-account detection can relax the tier in one place.
 */
export const SAFE_FLOOR = {
  freeAccountExtraPauseSec: 5
} as const;
...
export class RateLimiter {
  ...
  static calculateFilePause(fileSizeKb: number, speedMb: number, extraPauseSec: number): number {
    ...
    const effectiveExtraPauseSec = Math.max(extraPauseSec, SAFE_FLOOR.freeAccountExtraPauseSec);
    return computedPause + effectiveExtraPauseSec;
  }

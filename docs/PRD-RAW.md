# PRD-RAW — nextended (originally drafted as "Nexus Suite")

> AMENDMENTS applied to this copy (user decisions, 2026-08-24):
> - Extension name is **nextended** (replaces "Nexus Suite" everywhere; decision D1 overridden).
> - Storage keys: `nextended.v1.*` (was `nsuite.v1.*`), quarantine keys `nextended.corrupt.<ts>`.
> - Log tag: `[Nextended]`. Shadow host element: `<nextended-root>`. Marker attributes: `data-nextended-*`.
> - §5 of the original draft was truncated mid-way through the RESOLVER strategy chain item;
>   it is completed in `docs/BUILD_PROMPT.md` from FR3. This file preserves the raw source.
> - No reference userscripts are available in the workspace. All behaviour below is the
...
|---|----------|----------|-----|
| D1 | Name | **nextended** | User-selected name (overridden from "Nexus Suite") |
...
| D8 | Script C's fate | Deleted as a module | It is a strict subset of Script B's archive handler; keep only its ModRequirementsPopUp URL format as a legacy-markup fallback inside the archive module |
...
|------|----------------------------------------|
| Feature parity for daily workflows | Parity checklist (§5.6) 100% green |
...
- **Selector quarantine:** every selector, URL pattern, regex, and GraphQL query lives in `src/core/siteAdapters.ts`. An ESLint rule (`no-restricted-syntax` on regex literals matching "nexus") enforces it.
...
|------|----------|------------|
| Nexus ToS / store rejection | High | D14 distribution order; D16 anti-bypass line; budget system as good-citizen evidence; be prepared to keep this GitHub-only |
| Nexus HTML/GraphQL drift | High, recurring | Selector quarantine + fixture-based tests + a "site adapter version" log line so bug reports are diagnosable |
| MV3 worker lifecycle corrupting state | Medium | D7 (loop in content), storage-first state machine, Zod validation, resume banner |
| Shadow-DOM component changes breaking interception | Medium | Strategy chain degrades gracefully; failures classify as `unresolved` with the native UI still clickable |
...
### 2.8 Acceptance tests (definition of done — live-site, OUT of autonomous scope)
...
- T5 Worker kill: serviceworker stop mid-run → run continues (loop is content-side) and next budget:spend succeeds after worker cold-start.
  ...
  src/
    ...
    content/
      ...
      ui/
        ...
        toast.ts
        components/             // panel, progress, log, modal (vanilla)
      modules/
        collection/
        ...
        nowait/
          index.ts
          intercept.ts          // click capture, shadow-DOM walk
          autostart.ts
        archive/
          index.ts
    core/
      ...
      types.ts
    options/                    // Preact
    popup/                      // Preact: budget status, active run glance, debug toggle
  fixtures/                     // captured HTML/JSON for tests
    collection-revision.json
    modpage-with-regex-url.html
    modpage-component-attrs.html
    generate-download-url.json
    cloudflare-challenge.html
    logged-out.html
    suspended.html
    archived-legacy.html
    archived-current.html
  tests/                        // Vitest, mirrors core/
```

### 3.2 Contracts worth pinning
- **Resolver** — `resolve(input: ResolveInput, ctx: {isNMM: boolean; signal: AbortSignal}) → { ok: true; url; strategy } | { ok: false; error: ErrorClass; evidence: string }`. Pure orchestration; all fetches injected via a `HttpClient` interface so tests run against fixtures with zero network. `evidence` is the first 500 chars of the offending response, debug-mode only, for the log console's copy button.
...
| history.pushState monkey-patching | Same technique, but injected once by the router in the MAIN world via chrome.scripting registered content script (world: "MAIN" for the hook, bridged by CustomEvent to the isolated world) — MV3 isolated worlds can't see page-side history patching otherwise |

That last row is the one non-obvious MV3 trap in this whole port; budget a day for it.
...
- Capture fixtures (synthesised from the behaviour descriptions here, since the original userscripts are unavailable): GraphQL responses (small + large collection), mod pages in every markup variant (regex-era, component-era), GenerateDownloadUrl responses, logged-out page, Cloudflare page, suspension page, both archived-page markups.
...
- Router incl. the MAIN-world history hook bridge (the trap from §3.3).
...
- GraphQL client, panel, selection modal (virtualised list), run engine with full persistence, history + skip-downloaded modal, log console, resume banner.
...
- Options page completeness + settings import/export; popup (budget glance, active run, debug toggle); empty/error states; icon set.
...
## 5. Build prompt draft
**TRUNCATED in the original source** — it cut off at the RESOLVER strategy-chain item.
`docs/BUILD_PROMPT.md` is the completed reconstruction; its STACK, LAYOUT, and
ARCHITECTURE RULES sections follow §3 verbatim, with the resolver rule completed from FR3
(all seven strategies, in order, with `{strategy, elapsedMs}` recording and FR6
